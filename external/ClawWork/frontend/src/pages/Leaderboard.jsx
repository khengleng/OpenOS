...
const LastUpdated = ({ lastFetchTime }) => {
  ...
  const secs = Math.max(0, Math.floor((now - lastFetchTime) / 1000))
  return (
...
const Leaderboard = ({ hiddenAgents = new Set() }) => {
  ...
  const handleResizerMouseDown = (e) => {
    ...
    const onMove = (ev) => {
      ...
      setChartFlexRatio(Math.min(75, Math.max(15, startRatio + delta)))
    }
    const onUp = () => {
          ...
          <table className={`w-full text-sm${isFullscreen ? ' fs-compact' : ''}`}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.03)' }}>
                ...
                <DarkSortHeader label="Starter"     sortKey="initial_balance"   currentKey={sortKey} onSort={handleSort} />
                <DarkSortHeader label="Balance"     sortKey="current_balance"   currentKey={sortKey} onSort={handleSort} />
                <DarkSortHeader label="% Change"    sortKey="pct_change"        currentKey={sortKey} onSort={handleSort} />
                <DarkSortHeader label="Income"      sortKey="total_work_income" currentKey={sortKey} onSort={handleSort} />
                <DarkSortHeader label="Cost"        sortKey="total_token_cost"  currentKey={sortKey} onSort={handleSort} />
                ...
                <DarkSortHeader label="Avg Quality" sortKey="avg_eval_score"    currentKey={sortKey} onSort={handleSort} />
                <DarkSortHeader label="Tasks"       sortKey="num_tasks"         currentKey={sortKey} onSort={handleSort} />
...
const DarkSortHeader = ({ label, sortKey, currentKey, onSort }) => (
  <th
